...
#define from_prcg(pctg) (int)((((256)*pctg)/100)/2.5)


int main(void)
{
	MotorFD mymot = motor_begin(15,14,13);
	usart_begin(UBRR_VAL);
	stdout = &usart_stdout;
	stdin = &usart_stdout;
	int value = 0,_value;
	float mot1calib = 0.934579439;
	while(1)
	{
		/*
		_value = usart_getc();
		...
		*/
		/*
		motor_calib_drive(mymot,28);
		_delay_ms(5000);
		set_motor(mymot, (127));
		_delay_ms(2000);
		motor_stop(mymot,127);
		_delay_ms(5000);
		*/
		motor_calib_drive(mymot,28);
		motor_bsdrive_mm(mymot,20,mot1calib,2);
		_delay_ms(1000);
		motor_bsdrive_mm(mymot,12,mot1calib,5);
		_delay_ms(1000);
